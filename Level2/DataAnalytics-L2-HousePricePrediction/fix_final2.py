...
new_source = []
for line in source:
    if line.strip() == "metrics = evaluate_model(y_test, y_pred, 'Test_')    metrics['Model'] = name":
        new_source.append("    metrics = evaluate_model(y_test, y_pred, 'Test_')")
        new_source.append("    metrics['Model'] = name")
    elif line.strip() == "metrics = evaluate_model(y_test, y_pred)":
        new_source.append("    metrics = evaluate_model(y_test, y_pred, 'Test_')")
    else:
